...
public class FCFSSimulatorService {
    ...
    private List<FCFSOutput> runFCFSSimulator(SimulatorInput simulatorInput, Map<Long, Long> initialSupply) {
        ...
        for (SimulatorInput.SimulatorInputEntity simulatorInputEntity : simulatorInputEntityList) {
            outputMap.putIfAbsent(simulatorInputEntity.getFromStation(), initialSupply.get(simulatorInputEntity.getFromStation()));
            if (outputMap.containsKey(simulatorInputEntity.getFromStation())
                    && outputMap.get(simulatorInputEntity.getFromStation()) > 0) {
                outputMap.put(simulatorInputEntity.getFromStation(), outputMap.get(simulatorInputEntity.getFromStation()) - 1);
            }
        }
